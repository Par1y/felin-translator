...
/// + instruction (per-item, higher priority than 总则) + optional context
/// (previous approved translation, for style/naming continuity) + source.
///
/// Glossary-hit injection is added by the names/pipeline steps.
...
mod tests {
    ...
    #[test]
    fn assembles_system_instruction_context_source_in_order() {
        ...
        assert!(m[2].content.contains("前文译文。"));
        assert!(m[2].content.contains("本文。"));
    }

    #[test]
    fn omits_empty_system_instruction_and_context() {
